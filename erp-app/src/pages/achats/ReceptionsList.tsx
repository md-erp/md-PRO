  ...
  return (
    <div className="h-full flex flex-col gap-3">
      <div className="flex items-center gap-3">
        <button className="btn-primary" onClick={() => setModalOpen(true)}>+ Nouveau Bon de Réception</button>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            {!loading && (data?.rows.length ?? 0) === 0 && (
              <tr><td colSpan={6} className="text-center py-16">
                ...
                <div className="text-gray-500">Aucun bon de réception</div>
              </td></tr>
            )}
            {data?.rows.map(doc => (
              <tr key={doc.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                ...
                <td className="px-4 py-3 text-center">
                  <span className={doc.status === 'confirmed' ? 'badge-green' : 'badge-orange'}>
                    {doc.status === 'confirmed' ? '✅ Appliqué' : '⏳ En attente'}
                  </span>
                </td>
