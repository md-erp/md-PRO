  ...
  return (
    <div className="h-full flex flex-col gap-3">
      <div className="flex items-center gap-3">
        <button className="btn-primary" onClick={() => setModalOpen(true)}>+ Nouvelle Facture Fournisseur</button>
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 dark:bg-gray-700/50 sticky top-0">
            <tr>
              ...
              <th className="px-4 py-3 text-right font-medium text-gray-600">Total HT</th>
              <th className="px-4 py-3 text-right font-medium text-gray-600">Total TTC</th>
              <th className="px-4 py-3 text-center font-medium text-gray-600">Paiement</th>
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
            ...
            {!loading && (data?.rows.length ?? 0) === 0 && (
              <tr><td colSpan={7} className="text-center py-16">
                ...
                <div className="text-gray-500">Aucune facture fournisseur</div>
              </td></tr>
            )}
            {data?.rows.map(doc => (
              <tr key={doc.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30">
                ...
                <td className="px-4 py-3 text-center">
                  <span className={PAY_STATUS[doc.status] ?? 'badge-gray'}>
                    {doc.status === 'paid' ? '✅ Payée' : doc.status === 'partial' ? '⚡ Partiel' : '❌ Impayée'}
                  </span>
                </td>
