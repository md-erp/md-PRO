...
// CUSTOM HTML ENGINE
// ════════════════════════════════════════════════════════════════════
function applyCustomTemplate(tpl, data) {
    ...
    const vars = {
        ...
        '{{lines_html}}': lines.map((l) => `<tr><td>${l.product_name ?? l.description ?? '—'}</td>` +
            `<td>${l.quantity}</td><td>${fmt(l.unit_price)}</td>` +
            `<td>${fmt(l.total_ht ?? l.quantity * l.unit_price)}</td></tr>`).join(''),
    };
    let result = tpl;
...
// ════════════════════════════════════════════════════════════════════
function generateMasterTemplate(data) {
    ...
    const logoW = company?.company_logo_width ?? 0;
    const logoH = company?.company_logo_height ?? 0;
    const ratio = logoH > 0 ? logoW / logoH : 2;
    ...
    // ─── Client badges ────────────────────────────────────────────────
    const badge = (label, val) => `<span style="display:inline-block;padding:3px 10px;border:1px solid ${BORDER};border-radius:20px;font-size:10px;color:#444;margin:2px 3px 0;background:#fff;"><strong>${label}:</strong> ${val}</span>`;
    const clientBadges = [
