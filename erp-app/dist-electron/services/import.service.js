...
// ==========================================
function translateError(rawError, entityType, row) {
...
// ==========================================
async function importClients(filePath, userId = 1) {
    const entityType = 'client';
    const db = (0, connection_1.getDb)();
    const result = { success: 0, errors: [], total: 0 };
  ...
  `);
    const tx = db.transaction(() => {
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                stmt.run(name, String(row['Adresse'] ?? row['address'] ?? '').trim() || null, String(row['Email'] ?? row['email'] ?? '').trim() || null, String(row['Téléphone'] ?? row['Telephone'] ?? row['phone'] ?? '').trim() || null, String(row['ICE'] ?? row['ice'] ?? '').trim() || null, String(row['IF'] ?? row['if'] ?? '').trim() || null, String(row['RC'] ?? row['rc'] ?? '').trim() || null, String(row['Notes'] ?? row['notes'] ?? '').trim() || null, userId);
                result.success++;
            }
            catch (e) {
...
// ==========================================
async function importSuppliers(filePath, userId = 1) {
...
// ==========================================
async function importProducts(filePath, userId = 1) {
    ...
    result.total = rows.length;
    const tvaRates = db.prepare('SELECT id, rate FROM tva_rates').all();
    const tvaMap = {};
  ...
  `);
    const tx = db.transaction(() => {
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                stmt.run(code, name, String(row['Unité'] ?? row['Unite'] ?? row['unit'] ?? 'unité').trim(), validType, isNaN(minStock) ? 0 : minStock, salePrice, tvaId, String(row['Notes'] ?? row['notes'] ?? '').trim() || null, userId);
                result.success++;
            }
            catch (e) {
                result.errors.push({ row: rowNum, message: translateError(e.message, entityType, row) });
...
// ==========================================
async function readExcelOrCsv(filePath) {
...
function parseCsv(filePath) {
    ...
    const headers = lines[0].split(sep).map(h => h.trim().replace(/^"|"$/g, '').replace(/\s*\*\s*$/, '').trim());
...
// ==========================================
async function generateImportTemplate(type) {
    const wb = new exceljs_1.default.Workbook();
