...
function registerProductionHandlers() {
    // BOM Templates
    (0, index_1.handle)('production:getBoms', (productId) => {
      ...
      GROUP BY bt.id
    `).all(productId);
        return boms.map(b => ({ ...b, lines: JSON.parse(b.lines ?? '[]').filter((l) => l.id) }));
    });
    (0, index_1.handle)('production:createBom', (data) => {
        const db = (0, connection_1.getDb)();
    ...
    // Production Orders
    (0, index_1.handle)('production:getAll', (filters) => {
        const db = (0, connection_1.getDb)();
        return db.prepare(`
    ...
    (0, index_1.handle)('production:create', (data) => {
        const db = (0, connection_1.getDb)();
        const tx = db.transaction(() => {
            // حساب التكلفة من BOM
            let unit_cost = 0;
            ...
            if (data.bom_id) {
                const bom = db.prepare('SELECT * FROM bom_templates WHERE id = ?').get(data.bom_id);
                const lines = db.prepare('SELECT bl.*, p.cmup_price FROM bom_lines bl JOIN products p ON p.id = bl.material_id WHERE bl.bom_id = ?').all(data.bom_id);
                const materials_cost = lines.reduce((sum, l) => sum + l.quantity * l.cmup_price, 0);
        ...
        VALUES (?, ?, ?, ?, ?, 'draft', ?, ?, ?, ?)
      `).run(data.product_id, data.bom_id ?? null, bom_snapshot, data.quantity, data.date, unit_cost, unit_cost * data.quantity, data.notes ?? null, data.created_by ?? 1);
            return { id: result.lastInsertRowid, unit_cost, total_cost: unit_cost * data.quantity };
        });
        return tx();
    });
    (0, index_1.handle)('production:confirm', (id, userId = 1) => {
        const db = (0, connection_1.getDb)();
        ...
        const tx = db.transaction(() => {
            ...
            const bom = order.bom_id ? db.prepare('SELECT labor_cost FROM bom_templates WHERE id = ?').get(order.bom_id) : null;
            ...
            for (const line of bom_lines) {
                ...
                (0, stock_service_1.applyMovement)(db, movId, userId);
            }
            ...
            (0, accounting_service_1.createAccountingEntry)(db, fakeDoc, [], userId);
    ...
    // Transformations
    (0, index_1.handle)('transformations:getAll', () => {
        const db = (0, connection_1.getDb)();
        return db.prepare(`
    ...
    (0, index_1.handle)('transformations:create', (data) => {
        const db = (0, connection_1.getDb)();
        const tx = db.transaction(() => {
            // حساب التكلفة الإجمالية
            const material = db.prepare('SELECT * FROM products WHERE id = ?').get(data.raw_material_id);
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `).run(data.raw_material_id, data.input_quantity, data.cost_per_unit ?? 0, total_cost, data.date, data.notes ?? null, data.created_by ?? 1);
            ...
            for (const output of outputs) {
                ...
                (0, stock_service_1.applyMovement)(db, movId, data.created_by ?? 1);
            }
            // حركة مخزون للمادة الأولية (خروج)
            const outMovId = (0, stock_service_1.createStockMovement)(db, {
            ...
            (0, stock_service_1.applyMovement)(db, outMovId, data.created_by ?? 1);
